...
namespace
{
    ...
    // rows is: Font, Language, Resolution, Frame rate, Windowed mode (combos
    // grouped at the top so an open dropdown never overlaps the Close button).
    ...
    // Frame-rate combo box placement (relative to m_Pos), one row below the
    // resolution row. The Windowed-mode row and everything below it (Close
    // button, frame bottom) shift down by 39 px to make room.
    constexpr int FPS_LABEL_Y_LOCAL = 361;
    constexpr int FPS_COMBO_X_LOCAL = 22;
    constexpr int FPS_COMBO_Y_LOCAL = 374;
    constexpr int FPS_COMBO_WIDTH   = 148;
    constexpr int FPS_COMBO_HEIGHT  = 16;
    constexpr int FPS_COMBO_MAX_VISIBLE = 5;

    // Frame-rate presets offered by the combo. 0 = monitor refresh rate,
    // -1 = unlimited; positive values are fixed ceilings. Keep the order in
    // sync with the combo label builder in InitFpsCombo().
    constexpr int FPS_PRESET_MONITOR = 0;
    constexpr int FPS_PRESET_UNLIMITED = -1;
    const int s_FpsPresets[] = { 30, 60, 90, 120, 144, 180, FPS_PRESET_MONITOR, FPS_PRESET_UNLIMITED };
    constexpr int s_NumFpsPresets = sizeof(s_FpsPresets) / sizeof(s_FpsPresets[0]);
}
...
void SEASON3B::CNewUIOptionWindow::InitFpsCombo()
{
    ...
    m_fpsLabels.clear();
    m_fpsLabels.reserve(s_NumFpsPresets);
    for (int preset : s_FpsPresets)
    {
        if (preset > 0)
        {
            m_fpsLabels.push_back(std::to_wstring(preset) + L" FPS");
        }
        else if (preset == FPS_PRESET_MONITOR)
        {
            // Show the refresh rate the preset would actually cap at, so the
            // entry reads e.g. "Monitor maximum (144 Hz)".
            m_fpsLabels.push_back(std::wstring(I18N::Game::MonitorMaximum) + L" (" +
                                  std::to_wstring(GetFPSLimit()) + L" Hz)");
        }
        else
        {
            m_fpsLabels.push_back(std::wstring(I18N::Game::Unlimited));
        }
    }

    m_fpsLabelPointers.clear();
    m_fpsLabelPointers.reserve(m_fpsLabels.size());
    for (const std::wstring& label : m_fpsLabels)
    {
        m_fpsLabelPointers.push_back(label.c_str());
    }
...
void SEASON3B::CNewUIOptionWindow::SetButtonInfo()
{
    ...
    m_BtnClose.ChangeButtonInfo(m_Pos.x + 68, m_Pos.y + 427, 54, 30);
...
bool SEASON3B::CNewUIOptionWindow::UpdateMouseEvent()
{
    ...
    const ComboSlot slots[] = {
        ...
        { &m_FpsCombo,        &m_iFpsIndex,        &CNewUIOptionWindow::ApplyFps         },
    };
...
void SEASON3B::CNewUIOptionWindow::OpenningProcess()
{
    ...
    m_FontCombo.Close();
    // Rebuild the FPS combo so "Monitor maximum (N Hz)" reflects the display
    // the window is on right now (it can change between opens).
    InitFpsCombo();
    m_FpsCombo.Close();
    m_bWindowedMode = (g_bUseWindowMode == TRUE);
}
...
void SEASON3B::CNewUIOptionWindow::RenderContents()
{
    ...
    y += 39.f;
    RenderImage(IMAGE_OPTION_POINT, x, y, 10.f, 10.f);       // Frame rate
    g_pRenderText->RenderText(m_Pos.x + 40, m_Pos.y + FPS_LABEL_Y_LOCAL, I18N::Game::FrameRate);

    y += 39.f;
    RenderImage(IMAGE_OPTION_POINT, x, y, 10.f, 10.f);       // Windowed Mode
    g_pRenderText->RenderText(m_Pos.x + 40, m_Pos.y + 400, I18N::Game::WindowedMode);
}
...
void SEASON3B::CNewUIOptionWindow::RenderButtons()
{
    ...
    CNewUIComboBox* combos[] = { &m_ResolutionCombo, &m_LanguageCombo, &m_FontCombo, &m_FpsCombo };
