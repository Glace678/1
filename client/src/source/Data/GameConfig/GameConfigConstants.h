...
namespace CfgKeys
{
    ...
    // Frame-rate preset. 0 = follow the monitor (cap at the display refresh
    // rate), -1 = uncapped, any positive value (30/60/90/120/144/180/...) is a
    // fixed FPS ceiling applied on top of VSync.
    inline constexpr wchar_t CfgKeyFpsPreset[] = L"FpsPreset";
}
...
namespace CfgDefaults
{
    ...
    // 0 = monitor refresh rate (the historical behaviour).
    inline constexpr int CfgDefaultFpsPreset = 0;
}
