...
void MuApplyWindowResolution(unsigned int width, unsigned int height, bool windowed)
{
    ...
    // The monitor refresh rate may differ on the new display/mode, so re-arm
    // the FPS preset too ("monitor maximum" must track the actual display).
    MuReapplyFpsPreset();
}
